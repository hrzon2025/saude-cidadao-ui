  ...
  return <div className="min-h-screen bg-background pb-20">
      ...
      <div className="max-w-md mx-auto p-4 space-y-6">
        ...
        <Card className="p-4">
          <div className="space-y-4">
            ...
            {loadingHorarios ? <div className="space-y-2">
                <SkeletonCard />
                <SkeletonCard />
              </div> : horariosDisponiveis_.length === 0 ? <div className="text-center py-8">
                ...
                {horariosIndisponiveis > 0 && <p className="text-xs text-muted-foreground">
                    {horariosIndisponiveis} horário{horariosIndisponiveis > 1 ? 's já ocupados' : ' já ocupado'}
                  </p>}
              </div> : <>
                <div className="grid grid-cols-3 gap-2">
                  {horariosDisponiveis.map(horario => <Button key={horario.hora} variant={horarioSelecionado === horario.hora ? "default" : "outline"} size="sm" disabled={!horario.disponivel} onClick={() => setHorarioSelecionado(horario.hora)} className={cn("h-10 text-xs", !horario.disponivel && "opacity-40 cursor-not-allowed", horarioSelecionado === horario.hora && "bg-primary text-primary-foreground")}>
                      {horario.hora}
                    </Button>)}
                </div>
                
                {horariosIndisponiveis > 0 && <p className="text-xs text-muted-foreground text-center">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={handleContinuar} disabled={!horarioSelecionado} className="flex-1">
            Continuar
