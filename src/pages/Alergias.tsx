...
export default function Alergias() {
  ...
  const fetchAlergias = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('alergias').select('*').eq('usuario_id', usuario?.id).order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  if (loading) {
    return <div className="min-h-screen bg-background">
        <AppHeader title="Alergias" showBack onBack={() => navigate("/minha-saude")} className="bg-primary" />
        ...
        <Card className="bg-card p-6 space-y-6">
          ...
          {isEditing && <div className="space-y-3">
              ...
              <div className="space-y-3">
                <Input value={novaAlergia.nome} onChange={e => setNovaAlergia(prev => ({
              ...prev,
              nome: e.target.value
            }))} placeholder="Nome da alergia (ex: Penicilina, Amendoim...)" className="w-full" />
                <Input value={novaAlergia.descricao} onChange={e => setNovaAlergia(prev => ({
              ...prev,
              descricao: e.target.value
            }))} placeholder="Descrição (opcional)" className="w-full" />
                
                <Button onClick={handleSave} disabled={!novaAlergia.nome.trim()} className="w-full bg-primary hover:bg-primary/90">
                  Salvar Alergia
                </Button>
              </div>
            </div>}
          ...
          <div className="space-y-4">
            ...
            {/* Alergias List */}
            <div className="space-y-3">
              {alergias.length === 0 ? <div className="text-center py-8 space-y-3">
                  ...
                  <div>
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleRemoveAlergia(alergia.id)} className="text-destructive hover:text-destructive/80 hover:bg-destructive/10">
